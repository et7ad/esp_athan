...
The DFPlayer itself ignores file names; it numbers audio files by walking the card's root directory slot by slot (32 bytes per entry) in physical order. See the README’s SD‑card section for user‑facing instructions.

**The root directory must fit in its first block (the "Fajr 7 and 8 don't work" bug, 2026‑09‑15).** On macOS every file created on a FAT volume gets an AppleDouble twin `._name` carrying `com.apple.provenance`; `cp -X`, `cat >`, `touch` and `mkdir` all trigger it (verified on macOS 26), and deleting the twin leaves its two slots dead. Each audio file therefore costs three slots: 48 files plus the two marker files = 165 slots. A FAT32 card with 4 KB clusters (Disk Utility's and Windows' default up to 8 GB) has 128 slots in the first root‑directory cluster, and the cheap modules do not follow the directory into its second cluster, so tracks 41–48 (F7–F10 and `Z_fallback`) are unreachable although Finder lists them. V5 with 38 files used 123 slots and stayed just under the limit. `scripts/sd_card_cpy_script.sh` now (1) formats the card itself: FAT16 up to 2 GB (fixed 512‑slot root directory), FAT32 with 32 KB clusters above (1024 slots per cluster); (2) copies, cleans, unmounts; (3) parses the raw FAT root directory from `/dev/rdiskNsM` with an embedded python3 snippet and prints every audio entry with its slot number in the module's order, failing on any entry beyond the first block, any `._` twin counted as a track, any order mismatch, or a folder with an audio name. `INSPECT=yes` runs only step 3 on an existing card, read‑only. Raw devices need 4 KB‑aligned reads; on macOS the console user can read and `newfs_msdos` removable devices without sudo (the script falls back to sudo). On Linux `DEST` is ignored: the script lists removable disks (`lsblk`, RM=1 or usb/mmc transport, never the disk behind `/`), asks which one is the card (or takes `DISK=/dev/sdX`), confirms, unmounts, `wipefs -a`, writes a one‑partition MBR with `sfdisk` (type `c` for FAT32, `6` for FAT16), `mkfs.fat -F 32 -s 64` (or `-F 16`), mounts it itself under a temp dir with `uid=`, copies, unmounts and runs the same check with `sudo` (block devices are root‑only), then `eject`. The Linux branch was written on a Mac and unit‑tested only with stubbed `lsblk`/`findmnt`; the first real run should be on a spare card. Simulating with disk images (`hdiutil create -fs "MS-DOS FAT32"`, run the script against `/Volumes/NAME`, dump the partition with `dd`, parse) reproduces a real card's layout exactly and is how to test changes without a card.
